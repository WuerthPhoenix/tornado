    ...
    async fn execute(&self, action: Rc<Action>) -> Result<(), ExecutorError> {
        ...
        match icinga2_action_result {
                ...
                if code.eq(ICINGA2_OBJECT_NOT_EXISTING_EXECUTOR_ERROR_CODE) =>
            {
                ...
                self.icinga_executor.perform_request(&icinga2_action).map_err(|err| ExecutorError::ActionExecutionError { message: format!("SmartMonitoringExecutor - Error while performing the process check result after the object creation. IcingaExecutor failed with error: {:?}", err), can_retry: err.can_retry(), code: None })
            }
            Err(err) => {
                error!(
