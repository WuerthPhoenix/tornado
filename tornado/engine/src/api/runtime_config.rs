...
impl RuntimeConfigApiHandlerImpl {
    pub fn new(logger_guard: Arc<LogWorkerGuard>, logger_level: String) -> Self {
        Self {
            logger_guard,
            logger_level: Arc::new(RwLock::new(logger_level))
        }
    }
}
...
mod test {
    ...
    async fn should_set_the_logger_level() {
        ...
        let api = RuntimeConfigApiHandlerImpl::new(log_guard, logger_level.clone());
