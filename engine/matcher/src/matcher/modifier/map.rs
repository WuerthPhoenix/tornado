...
pub fn map(
    ...
    if let Some(text) = value.get_text() {
        if let Some(mapped_value) = mapped_values.get(text) {
...
mod test {
    ...
    fn map_modifier_should_replace_a_string() {
        ...
        mapped_values.insert("Ok".to_owned(), "0".to_owned());
        mapped_values.insert("Warn".to_owned(), "1".to_owned());
        mapped_values.insert("Critical".to_owned(), "2".to_owned());

        {
            let default_value = None;
            let mut input = Value::Text("Ok".to_owned());
            map("", &mut input, &mapped_values, &default_value).unwrap();
    ...
    fn map_modifier_should_fail_if_input_not_string() {
...

        {
            ...
            assert!(map("", &mut input, &mapped_values, &default_value).is_err());
        }
    }
