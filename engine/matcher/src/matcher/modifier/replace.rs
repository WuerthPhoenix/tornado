...
mod test {
    ...
    fn replace_all_modifier_should_replace_a_string() {
...

        {
            ...
            replace_all(
                ...
                find_text,
                &replace_text,
                &(&event, &Value::Null, &Value::Null).into(),
            )
            .unwrap();
    ...
    //     {
    //         let mut input = Value::String("this is text".to_owned());
    //         replace_all("", &mut input, find_text, &replace_text, &(&event, &Value::Null, &Value::Null).into()).unwrap();
    ...
    fn replace_all_with_modifier_should_interpolate_extract_data_from_event_with_positional_groups()
    {
        ...
        let payload = {
            let mut payload = Map::new();
            payload.insert("role".to_owned(), Value::String("Great Bass Player".to_owned()));
            payload
        };
        let event = json!(Event::new_with_payload("my_type", payload));
