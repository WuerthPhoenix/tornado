...
pub fn metrics_endpoints(scope: Scope, metrics: Arc<Metrics>) -> Scope {
    scope
        .app_data(Data::new(metrics))
        .service(web::resource("text").route(web::get().to(text_metrics)))
}

async fn text_metrics(data: Data<Arc<Metrics>>) -> HttpResponse {
    let encoder = TextEncoder::new();
    let metric_families = data.prometheus_exporter.registry().gather();
    let mut buf = Vec::new();
