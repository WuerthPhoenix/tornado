...
#[tokio::test]
async fn get_trace_context_should_return_remote_span_id_if_sampled() {
    // Arrange
    ...
    let expected_span_id = "b7ad6b7169203331";
    let mut trace_context = serde_json::Map::new();
    trace_context.insert(
        "traceparent".to_owned(),
        Value::String(format!("00-{}-{}-01", expected_trace_id, expected_span_id)),
    );
    ...
    assert_eq!(context.span().span_context().span_id().to_hex().as_str(), expected_span_id);
    assert_eq!(
