use mailparse::{dateparse, parse_mail, MailParseError, MailHeaderMap};
use tornado_collector_common::{Collector, CollectorError};
use tornado_common_api::{Event, Value, Number};
...
impl<'a> Collector<&'a [u8]> for EmailEventCollector {
    fn to_event(&self, input: &'a [u8]) -> Result<Event, CollectorError> {
        ...
        let subject = email.headers.get_first_value("Subject").map_err(into_err)?.unwrap_or_else(|| "".to_owned());
        let date = dateparse(email.headers.get_first_value("Date").map_err(into_err)?.unwrap_or_else(|| "".to_owned()).as_str())
        ...
        event.payload.insert("body".to_owned(), Value::Text(body));
...
mod test {
    ...
    fn get_email(path: &str) -> String {
        fs::read_to_string(path)
            .expect(&format!("Unable to open the file [{}]", path))
    }

    fn get_event(path: &str) -> Event {
        let event_string = fs::read_to_string(path)
            .expect(&format!("Unable to open the file [{}]", path));
        serde_json::from_str(&event_string).expect(&format!("Cannot parse event from file [{}]", path))
    }
}
